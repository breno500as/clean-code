package com.cleancode.megasena;
...
public class Megasena {
	...
	/**
	 * Método que calcula o valor que uma certa aposta deve receber em função do
	 * número de acertos.
	 *
	...
	private boolean isApostaValida(List<Integer> numerosApostados) {
		return numerosApostados.size() >= 6 && numerosApostados.size() <= 15 && numerosApostados.stream().distinct()
				.filter(n -> n >= 1 && n <= 60).count() == numerosApostados.size();
	}
